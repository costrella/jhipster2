...
public class WeekResourceIntTest {
    ...
    public void initTest() {
        weekSearchRepository.deleteAll();
    ...
    public void createWeek() throws Exception {
        ...
        // Validate the Week in ElasticSearch
        Week weekEs = weekSearchRepository.findOne(testWeek.getId());
        assertThat(weekEs).isEqualToComparingFieldByField(testWeek);
    }
    ...
    public void updateWeek() throws Exception {
        // Initialize the database
        weekRepository.saveAndFlush(week);
        weekSearchRepository.save(week);
    ...
    public void deleteWeek() throws Exception {
        ...
        boolean weekExistsInEs = weekSearchRepository.exists(week.getId());
        assertThat(weekExistsInEs).isFalse();
    ...
    @Test
    @Transactional
    public void searchWeek() throws Exception {
        // Initialize the database
        weekRepository.saveAndFlush(week);
        weekSearchRepository.save(week);

        // Search the week
        restWeekMockMvc.perform(get("/api/_search/weeks?query=id:" + week.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(week.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME.toString())))
